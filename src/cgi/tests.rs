...
use super::response::{split_set_cookie_header, write_response_to};
use super::validation::{is_valid_header_name, is_valid_header_value};
use crate::common::{get_max_body_size, parse_query_string, Response};
...
fn test_write_response_multiple_set_cookie_lines() {
    ...
    assert_eq!(
        ...
        3,
        "expected 3 Set-Cookie lines, got: {}\n{}",
        set_cookie_lines.len(),
        out
    );
...
fn test_redact_value_for_log() {
    ...
    assert_eq!(
        redact_value_for_log("HTTP_HOST", "example.com"),
        "example.com"
    );
    ...
    assert_eq!(
        redact_value_for_log("HTTP_COOKIE", "session=abc123"),
        "***redacted***"
    );
    assert_eq!(
        redact_value_for_log("HTTP_X_API_KEY", "secret-key"),
        "***redacted***"
    );
...
fn test_redact_query_string() {
    ...
    // センシティブなパラメータがある場合
    assert_eq!(
        redact_query_string("name=john&token=secret123"),
        "name=john&token=***redacted***"
    );
    assert_eq!(
        redact_query_string("api_key=secret&user=admin"),
    ...
    // URLエンコードされた値
    assert_eq!(
        redact_query_string("secret=encoded%20value&name=test"),
        "secret=***redacted***&name=test"
    );

    // 値がない場合
    assert_eq!(
        redact_query_string("token=&name=john"),
...
fn test_gather_cgi_panic_context_no_headers() {
    ...
    // HTTPヘッダーが存在しない場合
    with_vars([("CONTENT_TYPE", Some("text/plain"))], || {
