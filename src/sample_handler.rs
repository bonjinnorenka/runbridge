...
impl Handler for EchoHandler {
    ...
    async fn handle(&self, req: Request) -> Result<Response, Error> {
        ...
        response_data.insert(
            "method".to_string(),
            serde_json::Value::String(format!("{}", req.method)),
        );
        response_data.insert(
            "path".to_string(),
...
mod tests {
    ...
    async fn test_path_nesting_priority() {
        ...
        // マッチするハンドラーを探す
        let handler = app
            .find_handler(&req_deep.path, &req_deep.method)
            .expect("Handler for /test/deep not found");
        ...
        let handler_shallow = app
            .find_handler(&req_shallow.path, &req_shallow.method)
            .expect("Handler for /test not found");
