...
fn assert_default_security_headers(headers: &std::collections::HashMap<String, String>) {
    assert_eq!(
        headers.get("X-Content-Type-Options").map(|s| s.as_str()),
        Some("nosniff")
    );
    assert_eq!(
    ...
    assert_eq!(
        headers.get("X-XSS-Protection").map(|s| s.as_str()),
        Some("1; mode=block")
    );
    assert_eq!(
        headers.get("Referrer-Policy").map(|s| s.as_str()),
...
fn response_allows_overrides() {
    ...
    assert_eq!(
        res.headers.get("X-Frame-Options").map(|s| s.as_str()),
        Some("SAMEORIGIN")
    );
    assert_eq!(
        res.headers
